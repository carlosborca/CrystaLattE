...
def rmin(nmer1, nmer2):
    """Takes to monomer files and returns separation between closest atoms belonging to different monomers."""
...
def main():
    ...
    rcut = 10.0 # WARNING: Interaction cuttoff is hardcoded.
    directory = os.getcwd()
    ...
    for mono1 in monofs:
        ...
        #if re.match('^1-36[0-9]+.xyz$', mono1):

            for mono2 in monofs:
                
                if re.match('^1-[0-9]+.xyz$', mono2):
                #if re.match('^1-36[0-9]+.xyz$', mono2):

                    if mono1 < mono2:
                        dimidx +=1
                        di = "2-" + str(mono1)[2:-4] + "+" + str(mono2)[2:]

                        mindist = rmin(mono1, mono2)
